...
Hutton's textbook[^hutton-textbook]:
- I added the solutions for exercises 1-5 of Chapter 2 to the repository (see the "Chapter 2" section of the README). I solved those tasks earlier on paper.  
Remark: I didn't expect the exercise 1 would take almost half a day to finish. I decided not just "run examples from the chapter in ghci" according to the task. Instead, I tried to notice and write down all interconnected concepts, abstractions and Haskell constructions that I had learned so far. Those appeared to be many things as I had worked through several chapters and dozen supplemental resources at that moment. Here are the code and explanations.
- Additional themes: `enumFromThenTo start_val (start_val + step) end_val` function. I found out about it occasionally while working on the same exercise 1 of Chapter 2.  
Here are several example with that function:
```
...
```
I have not decided where to write down this piece of knowledge and will leave it in the report for now. It should be easy to find this note in future using `ag` terminal utility.

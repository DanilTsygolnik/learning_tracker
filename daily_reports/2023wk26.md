...
Hutton's textbook[^hutton-textbook]:
- I added the solutions for exercises 1-5 of Chapter 2 to the repository. You can find them in the "Chapter 2" section of the README. These exercises were previously solved on paper.  

  Remark: Exercise 1 took much longer than expected. Instead of simply running the examples from the chapter in GHCi, as stated in the task, I decided to thoroughly analyze and document all the interconnected concepts, abstractions, and Haskell constructions that I had learned up to that point. Since I had covered several chapters and consulted multiple supplemental resources, there were numerous concepts to include. I have included the code and explanations in the repository.

- Additional topic: `enumFromThenTo`  

  I came across this function while working on exercise 1 of Chapter 2. It allows generating a list of values from a starting value to an ending value, with a specified step. Here are a few examples of its usage:
  ...
  I haven't decided where to permanently record this knowledge yet, so I will keep it in the report for now. I can easily search for this note in the future using the `ag` terminal utility.

[^hutton-textbook]: Reference to Hutton's textbook.

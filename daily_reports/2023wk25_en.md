...
`stack new` templates: added missing `hspec` to build dependency list in each file.

Textbook exercises projects:
1) planned project file structure;
2) updated `stack new` template accordingly;
3) updated Hutton's textbook project directory the same way: directory names, README;
4) added a solution for Hutton's chapter 1 exercise 1, updated README.

Miscellaneous

I added abbreviations to my Neovim config for several symbols frequently used in my markdown reports.

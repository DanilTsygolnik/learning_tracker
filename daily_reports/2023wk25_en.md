...
Using a good guide I had luckily found, I wrote the 1st unit test for Chapter's 1 exercise 2 (see above). GHC threw compilation error. That was no surprise for me, because I made a guess before that I should build the project first. Here's a couple of resources I've found for that:
  - official manual on Stack software installation;
  - Hspec documentation.

After I did some reading, I tried to create Stack project using `stack new tb-solutions` command. The command uses the `new-template.hsfiles` template by default. I dug into this as well and found useful information at the official wiki page. There I saw a reference to `kurt` template, so I searched for the author's textbook. I skimmed through the chapter 35 about building Stack project and chapter 36 dedicated to unit tests. The explanations seemed good, so I saved the book for further examination.
